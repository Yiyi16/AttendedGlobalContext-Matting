...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        alpha_weight = sample['alpha_weight']
        alpha_weight_fg = sample['alpha_weight_fg']
        # swap color axis because
        # numpy image: H x W x C
        # torch image: C X H X W
        image = image.transpose((2, 0, 1)).astype(np.float32)
        alpha = np.expand_dims(alpha.astype(np.float32), axis=0)
        alpha_weight = np.expand_dims(alpha_weight.astype(np.float32), axis=0)
        alpha_weight_fg = np.expand_dims(alpha_weight_fg.astype(np.float32), axis=0)
        trimap[trimap < 85] = 0
        trimap[trimap >= 170] = 2
        trimap[trimap >= 85] = 1
        ...
        sample['alpha_weight'] = torch.from_numpy(alpha_weight)
        sample['alpha_weight_fg'] = torch.from_numpy(alpha_weight_fg)
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        fg, alpha, alpha_weight, alpha_weight_fg,trimap, name = sample['fg'],  sample['alpha'], sample['alpha_weight'], sample['alpha_weight_fg'],sample['trimap'], sample['image_name']
        bg = sample['bg']
        h, w = trimap.shape
        ...
        if w < self.output_size[0]+1 or h < self.output_size[1]+1:
            ...
            while h < self.output_size[0]+1 or w < self.output_size[1]+1:
                ...
                alpha_weight = cv2.resize(alpha_weight, (int(w*ratio), int(h*ratio)),
                        interpolation=maybe_random_interp(cv2.INTER_NEAREST))
                alpha_weight_fg = cv2.resize(alpha_weight_fg, (int(w*ratio), int(h*ratio)),interpolation=maybe_random_interp(cv2.INTER_NEAREST))
        ...
        alpha_weight_crop = alpha_weight[left_top[0]:left_top[0]+self.output_size[0], left_top[1]:left_top[1]+self.output_size[1]]
        alpha_weight_fg_crop = alpha_weight_fg[left_top[0]:left_top[0]+self.output_size[0],left_top[1]:left_top[1]+self.output_size[1]]
        ...
        if len(np.where(trimap==128)[0]) == 0:
            ...
            alpha_weight_crop = cv2.resize(alpha_weight, self.output_size[::-1], interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            alpha_weight_fg_crop = cv2.resize(alpha_weight_fg, self.output_size[::-1], interpolation=maybe_random_interp(cv2.INTER_NEAREST))
        ...
        sample['fg'], sample['alpha'], sample['alpha_weight'], sample['alpha_weight_fg'],sample['trimap'] = fg_crop, alpha_crop, alpha_weight_crop, alpha_weight_fg_crop, trimap_crop
        sample['bg'] = bg_crop
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        sampel['image_origin'] = sample['image']
...
class OriginScale(object):
    def __call__(self, sample):
        ...
        sample['image_origin'] = sample['image']
...
class GenTrimap(object):
    ...
    def __call__(self, sample):
        ...
        #fg_weight = cv2.dilate(fg_mask, self.erosion_kernels[fg_width])
        ...
        sample['alpha_weight'] = fg_mask
        sample['alpha_weight_fg'] = 1-bg_mask
...
class DataGenerator(Dataset):
    ...
    def __getitem__(self, idx):
        if self.phase == "train":
            ...
            sample = {'fg': fg, 'alpha': alpha, 'alpha_weight':alpha, 'alpha_weight_fg':alpha,'bg': bg, 'fg_org':fg, 'bg_org':bg, 'trimap_org':bg, 'img_org':bg,'image_name': image_name}
        else:
            ...
            bg = cv2.imread(self.bg[idx], 1)
            sample = {'fg': fg, 'bg': bg, 'image': image, 'image_origin': image, 'alpha': alpha, 'alpha_weight': alpha, 'alpha_weight_fg':alpha,'trimap': trimap, 'image_name': image_name, 'alpha_shape': alpha.shape}

        sample = self.transform(sample)
        if self.phase == "train":
            sample['fg_org'] = cv2.imread(self.fg[idx % self.fg_num])
            sample['bg_org'] = cv2.imread(self.bg[idx], 1)
            sample['fg_org'] = cv2.resize(sample['fg_org'], (600, 600), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            sample['bg_org'] = cv2.resize(sample['bg_org'], (600, 600), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            sample['fg_org'] = cv2.cvtColor(sample['fg_org'], cv2.COLOR_RGB2BGR)
            sample['trimap_org'] = cv2.imread(self.alpha[idx % self.fg_num], 0).astype(np.float32)/255
            sample['trimap_org'] = cv2.resize(sample['trimap_org'], (600, 600), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            sample['img_org'] = sample['fg_org']*np.expand_dims(sample['trimap_org'], axis=2)+(1-np.expand_dims(sample['trimap_org'], axis=2))*sample['bg_org']
          
            alpha = sample['trimap_org']
            # Adobe 1K
            fg_width = np.random.randint(1, 5)
            bg_width = np.random.randint(1, 5)
            fg_mask = (alpha + 1e-5).astype(np.int).astype(np.uint8)
            bg_mask = (1 - alpha + 1e-5).astype(np.int).astype(np.uint8)
            fg_mask = cv2.erode(fg_mask, self.erosion_kernels[fg_width])
            bg_mask = cv2.erode(bg_mask, self.erosion_kernels[bg_width])
            ...
            trimap[fg_mask == 1] = 255
            trimap[bg_mask == 1] = 0
            sample['trimap_org'] = np.expand_dims(trimap, axis=2)
            
            mean = torch.tensor([0.485, 0.456, 0.406]).view(3,1,1)
            std = torch.tensor([0.229, 0.224, 0.225]).view(3,1,1)
            sample['img_org'] = torch.from_numpy(sample['img_org']/255.).permute(2,0,1).sub_(mean).div_(std)
            sample['fg_org'] = torch.from_numpy(sample['fg_org']/255.).permute(2,0,1).sub_(mean).div_(std)
            sample['bg_org'] = torch.from_numpy(sample['bg_org']/255.).permute(2,0,1).sub_(mean).div_(std)
        else:
            sample['img_org'] = img_org
            sample['img_org'] = cv2.resize(sample['img_org'], (600, 600), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            sample['img_org'] = cv2.cvtColor(sample['img_org'], cv2.COLOR_RGB2BGR)
            mean = torch.tensor([0.485, 0.456, 0.406]).view(3,1,1)
            std = torch.tensor([0.229, 0.224, 0.225]).view(3,1,1)
            sample['img_org'] = torch.from_numpy(sample['img_org']/255.).permute(2,0,1).sub_(mean).div_(std)
            
            alpha = trimap_org
            alpha = cv2.resize(alpha, (600, 600), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            fg_mask = (alpha + 1e-5).astype(np.int).astype(np.uint8)
            bg_mask = (1 - alpha + 1e-5).astype(np.int).astype(np.uint8)
            fg_mask = cv2.erode(fg_mask, self.erosion_kernels[3])
            bg_mask = cv2.erode(bg_mask, self.erosion_kernels[3])
