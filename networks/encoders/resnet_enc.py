...
class ResNet_D(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, late_downsample=False):
        ...
        self.conv1_trans = SpectralNorm(nn.Conv2d(3 + 1, 32, kernel_size=3,
                                            stride=self.start_stride[0], padding=1, bias=False))
        self.conv2_trans = SpectralNorm(nn.Conv2d(32, self.midplanes, kernel_size=3, stride=self.start_stride[1], padding=1,
                                            bias=False))
        self.conv3_trans = SpectralNorm(nn.Conv2d(self.midplanes, self.inplanes, kernel_size=3, stride=self.start_stride[2],
                                            padding=1, bias=False))
        self.bn1_trans = norm_layer(32)
        self.bn2_trans = norm_layer(self.midplanes)
        self.bn3_trans = norm_layer(self.inplanes)
        self.layer1_trans = self._make_layer(block, 64, layers[0], stride=self.start_stride[3])
        self.layer2_trans = self._make_layer(block, 128, layers[1], stride=2)

        self.conv1_transback = SpectralNorm(nn.Conv2d(3 + 1, 32, kernel_size=3,
                                            stride=self.start_stride[0], padding=1, bias=False))
        self.conv2_transback = SpectralNorm(nn.Conv2d(32, self.midplanes, kernel_size=3, stride=self.start_stride[1], padding=1,
                                            bias=False))
        self.conv3_transback = SpectralNorm(nn.Conv2d(self.midplanes, self.inplanes, kernel_size=3, stride=self.start_stride[2],
                                            padding=1, bias=False))
        self.bn1_transback = norm_layer(32)
        self.bn2_transback = norm_layer(self.midplanes)
        self.bn3_transback = norm_layer(self.inplanes)
        self.layer1_transback = self._make_layer(block, 64, layers[0], stride=self.start_stride[3])
        self.layer2_transback = self._make_layer(block, 128, layers[1], stride=2)
