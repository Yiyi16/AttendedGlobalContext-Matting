...
class ResTrans(ResNet_D):

    def __init__(self, block, layers, norm_layer=None, late_downsample=False):
        super(ResTrans, self).__init__(block, layers, norm_layer, late_downsample=late_downsample)
       
        first_inplane2 = 3 + 1
        self.shortcut_inplane2 = [first_inplane2, self.midplanes, 64, 128, 256]
        self.shortcut_plane2 = [32, self.midplanes, 64, 128, 256]

        self.shortcut2 = nn.ModuleList()
        for stage, inplane in enumerate(self.shortcut_inplane2):
            self.shortcut2.append(self._make_shortcut(inplane, self.shortcut_plane2[stage]))
    ...
    def forward(self, x, xtrans):
        out = self.conv1(x)
        out_trans = self.conv1_front(xtrans)
        ...
        if x.shape[1] == 6:
          ...
          fea1 = self.lateral1(self.shortcut2[0](x)) # input image and trimap
          fea2 = self.lateral2(self.shortcut2[1](x1))
          fea3 = self.lateral3(self.shortcut2[2](x2))
          fea4 = self.lateral4(self.shortcut2[3](x3))
          fea5 = self.lateral5(self.shortcut2[4](x4))
          out = self.lateral0(out)
