...
class ResGuidedCxtAtten_Dec(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False):
        ...
        self.conv1org = SpectralNorm(nn.ConvTranspose2d(self.midplanes, 32, kernel_size=4, stride=2, padding=1, bias=False))
        self.conv1bg = SpectralNorm(nn.ConvTranspose2d(self.midplanes, 32, kernel_size=4, stride=2, padding=1, bias=False))
        self.conv1fg = SpectralNorm(nn.ConvTranspose2d(self.midplanes, 32, kernel_size=4, stride=2, padding=1, bias=False))
        self.bn1 = norm_layer(32)
        self.leaky_relu = nn.LeakyReLU(0.2, inplace=True)
        self.conv2org = nn.Conv2d(32, 1, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2)
        self.conv2bg = nn.Conv2d(32, 3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2)
        self.conv2fg = nn.Conv2d(32, 3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2)
        ...
        self.inplanes = 64 if layers[0] > 0 else 32
        self.late_downsample = late_downsample
        self.midplanes = 64 if late_downsample else 32

        #self.layer1bg = self._make_layer(block, 256, layers[0], stride=2)
        #self.layer2bg = self._make_layer(block, 128, layers[1], stride=2)
        #self.layer3bg = self._make_layer(block, 64, layers[2], stride=2)
        self.layer4bg = self._make_layer(block, self.midplanes, layers[3], stride=2)
        
        self.inplanes = 64 if layers[0] > 0 else 32
        self.late_downsample = late_downsample
        self.midplanes = 64 if late_downsample else 32
        self.layer4fg = self._make_layer(block, self.midplanes, layers[3], stride=2)
    ...
    def forward(self, x, mid_fea):
        ...
        x, offset = self.gca(im, x, mid_fea['unknown'])
        x = self.layer3(x) +fea3# N x 64 x 128 x 128
        x0 = self.layer4(x) +fea2# N x 32 x 256 x 256
        
        #xbg = self.layer1(x)
        #xbg =  self.layer2bg(xbg)
        #xbg = self.layer3bg(xbg)
        ...
        bg = torch.sigmoid(xbg)
        #fg = torch.sigmoid(xfg)

        return alpha, bg, {'offset_1': mid_fea['offset_1'], 'offset_2': offset, 'offset_f': mid_fea['offset_f'], 'offset_b': mid_fea['offset_b']}
